...
interface T2ExcelImportProps {
  onImportSuccess: (result: any) => void;
  onImportError: (error: string) => void;
}

const T2ExcelImport: React.FC<T2ExcelImportProps> = ({ onImportSuccess, onImportError }) => {
  const { token } = useAuth();
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      formData.append('table', 'euk.ugrozeno_lice_t2'); // T2 tabela
      ...
      if (!response.ok) {
        ...
        onImportError(errorData.error || 'Greška pri importu T2 fajla');
        return;
      }
      ...
      if (result.status === 'SUCCESS') {
        ...
        onImportError(result.message || 'T2 Import failed');
      }
  ...
  return (
    <div className="t2-excel-import">
      ...
      >
        ...
        {isImporting ? 'T2 Import u toku...' : 'T2 Excel Import'}
        <input
          type="file"
          accept=".xlsx,.xls"
        ...
        <div className="fixed inset-0 bg-blue-500 bg-opacity-20 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 shadow-lg">
            <div className="text-center">
              ...
              <p className="text-lg font-semibold text-gray-900">Spustite T2 Excel fajl ovde</p>
