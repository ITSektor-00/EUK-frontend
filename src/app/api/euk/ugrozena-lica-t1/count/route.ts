...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/euk/ugrozena-lica-t1/count`, {
      method: 'GET',
      headers: {
    ...
    console.error('Greška pri dohvatanju broja T1 ugroženih lica:', error);
    return NextResponse.json(
      { error: 'Greška pri dohvatanju broja T1 ugroženih lica' },
      { status: 500 }
    );
  }
}
